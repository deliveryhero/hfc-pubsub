...
| `PUBSUB_TSCONFIG_PATH`                 | must be the path to your tsconfig preference file in case that ts files directory is pointed as `PUBSUB_ROOT_DIR`. its an optional variable and it could be entered by cli when `npx subscriptions start` or `npx subscriptions list` command is executed or default configuration applied for compilation. |
